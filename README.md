...
- 平均処理時間: 6,145ms
- 最大処理時間: 26,154ms
- ホールドアウトのブランド系列一致率: 35.0%
- ホールドアウトの製品完全一致Top1: 5.88%
- ホールドアウトの誤候補率: 0%
...
銘柄識別の本番入口は `identifyAlcoholProductPipeline()` に統一しています。OCR、バーコード、視覚参照、過去ログ、訂正辞書、複数写真を独立検索してから候補集合を一度だけ順位付けし、固定候補や自動確定は行いません。目視正解データは `tools/ground-truth-review/index.html` を直接開いて更新できます。画像本体は保存せず、JSON差分だけを出力します。

CIでは識別専用テスト、DBマイグレーション、クライアント依存ライセンスも独立して検証します。許可対象はMIT、ISC、Apache-2.0、MPL-2.0で、未確認ライセンスが追加された場合はデプロイ前に失敗します。
