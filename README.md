...
OCR改善の反復検証は `docs/ocr-validation.md` に記録しています。Google Driveの実画像72枚を検証し、画像本体はGitHubへ保存せず、ファイル名、MIME、サイズ、SHA-256、EXIF、OCR結果、信頼度、処理時間、成否のみを `tests/fixtures/google-drive-test-manifest.json` と `tests/results/ocr-*.json` に保存しています。

検証結果の概要:

- 対象: 72枚
- 内訳: HEIC/HEIF 58枚、JPEG 14枚
- HEIC変換成功率: 100%
- OCR文字取得率: 100%
- EXIF撮影日取得率: 87.5%
- 平均処理時間: 5,511ms
...
Google Driveの実画像72枚（HEIC/HEIF 58枚、JPEG 14枚）について、生レコードから `scripts/recalculate-validation-metrics.mjs` で再計算しています。画像本体はGitHubへ保存していません。

- OCR文字取得率: 100%
- 意味のある文字取得率: 75.0%
- 銘柄候補抽出率: 22.2%
- 候補なし: 77.8%
- 目視で正解を確定した14枚での正解候補包含率・top-1率: 35.7%
- 誤候補: 0件
...
文字を1文字以上取得しただけではOCR成功とは扱いません。正解値を目視で確定できない画像は `groundTruthStatus: unknown` とし、銘柄正解率の分母から除外します。候補抽出率22.2%は改善後も不十分で、未抽出77.8%を残課題として追跡します。

GitHub Actionsはinstall、typecheck、lint、unit test、integration test、銘柄ベンチマーク、buildの後、デプロイ前にproduction `dist` をWebKitとChromiumで検証します。72枚全体は保存済み実測結果で評価し、ブラウザE2Eでは各端末へHEIC 1枚とJPEG 1枚を本番と同じfile inputで渡します。このQAが失敗した場合はGitHub Pagesへデプロイしません。デプロイ後は公開URLのsmoke testを実行します。
