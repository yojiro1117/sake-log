...
GitHub Actionsはinstall、typecheck、lint、unit test、integration test、buildの後、デプロイ前にproduction `dist` をWebKitとChromiumで検証します。Drive画像20枚はrunnerの一時領域へ取得し、2回の10枚キューとして本番と同じfile inputへ渡します。このQAが失敗した場合はGitHub Pagesへデプロイしません。デプロイ後は公開URLのsmoke testを実行します。
