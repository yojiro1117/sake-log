...
条件: 変換後画像またはJPEG原画像を1200px幅へリサイズし、PSM 11でOCR。
...
詳細は `tests/results/ocr-cycle-2.json` に保存しています。第2回では `gray-contrast`、`center-crop-gray-contrast`、`threshold` を比較し、ノイズが多い画像では中央切り出しまたは二値化が採用されました。
...
正解値は画像から機械的に確定せず、`groundTruthStatus: unknown` としました。そのため、銘柄名の完全一致率・部分一致率は分母から除外し、`null` として記録しています。
...
4. EXIFと画像サイズを取得
5. 複数前処理パターンでOCR
6. 信頼度、文字数、候補一致数、ノイズ量で最良結果を選ぶ
7. 構造化候補マスターと照合する
8. 候補は自動確定せず、ユーザーが選択する
...
- 実画像72枚では文字取得はできたが、銘柄候補まで到達した割合は8.3%に留まる
- ラベル領域が小さい、反射が強い、筆文字、縦書き、ボトル曲面歪みでは候補抽出が弱い
- 正解値を画像から人手で確定していないため、完全一致率と部分一致率は算出していない
