...
const downloadResults = JSON.parse(await readFile(path.join(sourceDir, 'download-results.json'), 'utf8').catch(() => '[]'));
...
for (const [index, file] of selected.entries()) {
  const raw = await readFile(path.join(sourceDir, localNameById.get(file.driveFileId) ?? file.fileName));
  const bytes = /\.hei[cf]$/i.test(file.fileName) ? await convert({ buffer: raw, format: 'JPEG', quality: 0.7 }) : raw;
  const image = await Jimp.fromBuffer(bytes, { 'image/jpeg': { maxMemoryUsageInMB: 1024, maxResolutionInMP: 100 } });
  ...
  sheet.print({ font, x: x + 8, y: y + tileHeight + 6, text: `${offset + index + 1}. ${file.fileName} ${file.driveFileId.slice(0, 6)}`, maxWidth: tileWidth - 16 });
}
