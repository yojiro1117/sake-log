...
const records = payload.multiCrop.records.map((record) => {
...
payload.improvement = {
  top1AccuracyPoints: Number(((payload.multiCrop.summary.top1Accuracy - payload.legacy.summary.top1Accuracy) * 100).toFixed(2)),
  falsePositivePoints: Number(((payload.multiCrop.summary.falsePositiveRate - payload.legacy.summary.falsePositiveRate) * 100).toFixed(2))
};
payload.recalibratedAt = new Date().toISOString();
payload.recalibrationSource = 'Stored seen-before multi-crop similarity records from cycle 3; no image was decoded again.';
await writeFile(filePath, `${JSON.stringify(payload, null, 2)}\n`);
console.log(JSON.stringify(payload.multiCrop.summary));
